...
exports.releaseSite = function (sitename, message, assets, env) {
  ...
  var environments, assetTypes, invalidAssetTypes = [], dirname, filepaths;

  // environments for media server
  environments = ['eng', 'qa', 'prod'];

  // asset directories on the media server
  assetTypes = ['javascript', 'css', 'images'];
  ...
  filePaths = build.getAssetPaths(sitename, 'ftp', assets);
  ...
  if (cfg.notifications.release.sendEmails) {
    notify.send('release', {
      site: sitename,
      msg: message,
      files: build.getAssetPaths(sitename, 'ftp', assets)
    });
  }
