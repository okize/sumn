...
var siteList = sites.getMicrositeUrls('prod'),
    siteListLength = siteList.length,
    sortBy = 'name', // name, url, statusCode
    // inc = Math.ceil( (100/siteListLength) * 11) / 10, // amount to increment the progress bar by
    inc = 1,
    t = new table({
      head: [
        '',
...
var compareStatus = function (site) {
  ...
  var status,
      active = sites.isSiteActive( site.name );
